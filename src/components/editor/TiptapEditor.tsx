...
export function TiptapEditor({ editorRef, initialContent, className = '' }: TiptapEditorProps) {
  ...
  return (
    <div className={`bg-card/40 backdrop-blur-2xl border border-border/50 rounded-3xl overflow-hidden relative flex flex-col ${className}`}>
      
      {/* Editor Content (Full length, no internal scroll) */}
      <div className="flex-1 relative cursor-text text-foreground">
