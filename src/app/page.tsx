import { FileText, CheckSquare, Image as ImageIcon, Tv, TrendingUp, Users, ArrowRight } from "lucide-react";
import Link from "next/link";
import { DashboardSettings } from "@/components/DashboardSettings";

export default function Home() {
  return (
    <div className="flex flex-col min-h-screen pb-12 animate-in fade-in duration-500 max-w-[1200px] mx-auto w-full">
      <header className="mb-10">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-foreground">Overview Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {[
          ...
          <div 
            key={i} 
            className="group bg-card text-card-foreground p-5 rounded-xl border border-border flex flex-col justify-between transition- relative overflow-hidden"
          >
        ...
        <div className="bg-card text-card-foreground rounded-xl border border-border flex flex-col h-fit">
          ...
          <div className="p-6 flex flex-col gap-3">
            <Link href="/story-generate" className="w-full block">
              <div className="group w-full flex flex-col items-center justify-center gap-3 bg-primary text-primary-foreground hover:bg-primary/90 p-6 rounded-lg font-medium transition-colors">
                <FileText size={24} className="mb-1 opacity-90 group-hover:scale-110 transition-transform" /> 
                <span>Generate New Story</span>
              </div>
            </Link>
            <Link href="/script-generate" className="w-full block mt-2">
              <div className="group w-full flex flex-col items-center justify-center gap-3 bg-primary text-primary-foreground hover:bg-primary/90 p-6 rounded-lg font-medium transition-colors">
                <FileText size={24} className="mb-1 opacity-90 group-hover:scale-110 transition-transform" /> 
                <span>Generate New Script</span>
              </div>
            </Link>
            <Link href="/video-approval" className="w-full block mt-2">
